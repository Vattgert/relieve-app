...
const ActivityCard = ({
  activity: { id, title, date, image, host, country, city, votes },
  showRating = false,
}) => {
  ...
  const renderRating = (votes) => {
    ...
    return <span className={styles.activityRating}>{rating}</span>;
  };
        ...
        <div className={styles.author}>
          ...
          <div className={styles.authorName}>
            ...
            <span className={styles.name}>
              <Link href={`/profiles/${encodeURIComponent(hostId)}`}>
                <a>{hostFullName}</a>
              </Link>
            </span>
          </div>
